...
export const StrategyPnlChart: React.FC<Props> = ({ trades }) => {
  ...
  const getBarColor = (name: string, pnl: number) => {
    if (pnl < 0) return '#f59e0b';
    if (name.toLowerCase().includes('breakout')) return '#3b82f6';
    if (name.toLowerCase().includes('ema')) return '#10b981';
    return '#8b5cf6';
  };
      ...
      <div className="w-full h-56 mt-2 flex items-center justify-center">
        {data.length === 0 ? (
          <div className="text-center p-4">
            <BarChart3 className="w-8 h-8 text-slate-600 light:text-slate-400 mx-auto mb-2 opacity-60" />
            <p className="text-xs font-semibold text-slate-300 light:text-slate-700">No strategy data yet</p>
            <p className="text-[11px] text-slate-500 mt-0.5">Add trades with setups to compare performance</p>
          </div>
        ) : (
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data} margin={{ top: 10, right: 10, left: -10, bottom: 5 }}>
              ...
              <YAxis stroke="#64748b" fontSize={10} tickLine={false} axisLine={false} tickFormatter={(v) => `₹${v >= 1000 ? (v / 1000).toFixed(0) + 'k' : v}`} />
